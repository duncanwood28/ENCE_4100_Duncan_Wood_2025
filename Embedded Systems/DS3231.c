...
void DS3231_GetTime(I2C_HandleTypeDef *hi2c, DS3231_Time_t *t)
{
    t->seconds = readBCDRegister(hi2c, DS3231_SECONDS);
    t->minutes = readBCDRegister(hi2c, DS3231_MINUTES);
    t->hours   = readBCDRegister(hi2c, DS3231_HOURS);
    t->date    = readBCDRegister(hi2c, DS3231_DATE);
    t->month   = readBCDRegister(hi2c, DS3231_CEN_MONTH);
    t->year    = readBCDRegister(hi2c, DS3231_DEC_YEAR);
}

void DS3231_SetTime(I2C_HandleTypeDef *hi2c, DS3231_Time_t *t)
{
    writeBCDRegister(hi2c, DS3231_SECONDS,    t->seconds);
    writeBCDRegister(hi2c, DS3231_MINUTES,    t->minutes);
    writeBCDRegister(hi2c, DS3231_HOURS,      t->hours);
    writeBCDRegister(hi2c, DS3231_DATE,       t->date);
    writeBCDRegister(hi2c, DS3231_CEN_MONTH,  t->month);
    writeBCDRegister(hi2c, DS3231_DEC_YEAR,   t->year);
}
